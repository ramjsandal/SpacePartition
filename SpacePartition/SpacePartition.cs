...
public class SpacePartition
{
    ...
    SpacePartition GetRoot()
    {
        SpacePartition current = this;
    ...
    List<SpacePartition> GetNeighbors()
    {
        // get a map of neighboring spots and their directions
        Dictionary<Direction, (int, int)> neighborSpots = new Dictionary<Direction, (int, int)>();

        // Top Spot
        neighborSpots.Add(Direction.TOP, (xOrigin + 1, yOrigin - 1));

        // Left Spot
        neighborSpots.Add(Direction.LEFT, (xOrigin - 1, yOrigin + 1));

        // Bottom Spot
        neighborSpots.Add(Direction.BOTTOM, (xOrigin + 1, yOrigin + height + 1));

        // Right Spot
        neighborSpots.Add(Direction.RIGHT, (xOrigin + width + 1, yOrigin + 1));

        // Our Node's depth
        int targetDepth = GetDepth();

        List<SpacePartition> ret = new List<SpacePartition>();

        foreach (var pair in neighborSpots)
        {
            // Traversing from root of the tree 
            // select subarea that contains our spot recursively until
            // we hit our depth or a leaf
            SpacePartition root = GetRoot();

            // if our target coord is off the map ignore
            if (!root.ContainsCoordinate(pair.Value.Item1, pair.Value.Item2)) continue;

            SpacePartition current = root;

            while (current.GetDepth() < targetDepth)
            {
                // if its a root, this is our neighbor
                if (current.partitions.Count == 0)
                {
                    ret.Add(current);
                    break;
                }

                // if this isn't a root, find the subpartition that contains
                // our coordinate
                foreach (var partition in current.partitions)
                {
                    if (partition.Value.ContainsCoordinate(pair.Value.Item1, pair.Value.Item2))
                    {
            ...
            // if we already found our cell then move onto next spot
            if (current.GetDepth() == targetDepth) { break; }


            // if not, we want to get all subpartitions on the correct border with our target cell
            ret.Concat(current.GetAllSubpartitionsOnEdge(GetOppositeDirection(pair.Key)));
        }
